...
(async () => {
  ...
  try {
    const maxPages = 5; 
    let currentPage = 1;
    await driver.get('https://flowwow.com/moscow/cakes/');
    await driver.sleep(3000);
    ...
    while (hasNextPage && currentPage <= maxPages) {
      ...
      for (let product of products) {
        try {
          ...
          const image = cleanText(await product.findElement(By.css('.restricted img')).getAttribute('src'));
